...
class employeeLeave {
  ...
  applyLeave(payload, userId, fullName, email) {
    return new Promise((resolve, reject) => {
      ...
      const c=employeeLeaveModels.find({
        $and:
          [{"eid":userId},{"LeaveDate":payload.LeaveDate}
        ]})
      .then(d=>{
      ...
      {
        reject({ statusCode: 403,message:"return date is lesser then leave date" });
      }
        }
        else{
          reject({ statusCode: 403,message:"your retur or leave date has alredy been registerd" });
        }
      })
