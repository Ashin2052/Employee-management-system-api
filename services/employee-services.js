    ...
    return new Promise((resolve, reject) => {
      if (isadmin) {
        employee_Contact
          ...
          .then(d=> this.removeEmployeeLeave(userId, isadmin)
          .then(e=>resolve(e))
          .catch(c=>reject(c))
         )
          .catch(e => reject(e));
      ...
      employee_Contact.findById(userId).then(d => {
        ...
        if (existingEncryptedString == payload.currentpassword) {
          if (payload.newPassword) {
            ...
            reject({ statusCode: 500, message: "you haven't entered any new password" });
          }
        } else {
          reject({ statusCode: 500, message: "enter correct current password" });
        }
      });
    });
  }
