...
router.get("/getApproveDetails/:userId", validateUser, (req, res, next) => {
  ...
  if (req.params.userId == req.UserId) {
    employeefunction
      .getApproveDetails(req.params.userId)
      .then(d => res.json(d))
      .catch(next);
  } else {
...
router.put("/resetPassword", validateUser, (req, res, next) => {
  ...
  employeefunction
    .resetPassword(req.UserId, req.email, req.password, req.body)
    .then(d => res.json(d))
    .catch(next);
});
...
router.get("/:userId", validateUser, (req, res, next) => {
  employeefunction
    .findParticular(req.params.userId)
    .then(d => res.json(d))
    .catch(next);
});
...
router.delete("/:userId", validateUser, (req, res, next) => {
  if (req.isadmin == "true") {
    employeefunction
      .removeEmployee(req.params.userId)
      .then(d => res.json(d))
      .catch(next);
    employeefunction
      .removeEmployeeLeave(req.params.userId)
      .then(d => res.json(d))
      .catch(next);
  } else {
    res.json("you are not an admin");
